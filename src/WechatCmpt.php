...
class WechatCmpt
{
	...
	const MENU_CREATE_URL = '/menu/create?';			//微信菜单-创建菜单
	const MENU_GET_URL = '/menu/get?';					//微信菜单-获取自定义菜单配置
	const MENU_DELETE_URL = '/menu/delete?';			//微信菜单-删除菜单
	const MENU_CHECK_URL = '/get_current_selfmenu_info?';	//微信菜单-获取当前配置
	const MENU_ADDCONDITIONAL_URL = '/menu/addconditional?'; //微信菜单-创建个性化菜单
	const MENU_DELCONDITIONAL_URL = '/menu/delconditional?'; //微信菜单-删除个性化菜单
    ...
    const TEMPLATE_GET_ALL_TPL_URL = '/template/get_all_private_template?';

    const TEMPLATE_DEL_TPL_URL = '/template/del_private_template?';
	const TEMPLATE_SEND_URL = '/message/template/send?';
    const SUBSCRIBE_GET_TPL_CATEGORY_URL = '/wxaapi/newtmpl/getcategory?';
    const SUBSCRIBE_GET_TPL_KEYWORDS_URL = '/wxaapi/newtmpl/getpubtemplatekeywords?';
    const SUBSCRIBE_GET_TPL_TITLE_URL = '/wxaapi/newtmpl/getpubtemplatetitles?';
    const SUBSCRIBE_ADD_TPL_URL = '/wxaapi/newtmpl/addtemplate?';
    const SUBSCRIBE_GET_ALL_TPL_URL = '/wxaapi/newtmpl/gettemplate?';
    const SUBSCRIBE_DEL_TPL_URL = '/wxaapi/newtmpl/deltemplate?';
    const SUBSCRIBE_SEND_URL = '/message/subscribe/bizsend?';
    ...
    const DRAFT_BATCHGET_URL = '/draft/batchget?';
    const DRAFT_COUNT_URL = '/draft/count?';
    const DRAFT_ADD_URL = '/draft/add?';
    const DRAFT_GET_URL = '/draft/get?';
	...
	const CARD_MPNEWS_GETHTML               = '/card/mpnews/gethtml?';     //图文消息群发卡券
	const FREEPUBLISH_BATCHGET               = '/freepublish/batchget?';     //获取已发布图文
    ...
    //标签相关
    const TAGS_GET                      = '/tags/get?'; //获取已创建的标签
    const TAGS_CREATE                   = '/tags/create?'; //创建标签
    const TAGS_UPDATE                   = '/tags/update?'; //删除标签
    const TAGS_DELETE                   = '/tags/delete?'; //删除标签
    const USER_TAG_GET                  = '/user/tag/get?'; //获取标签下的粉丝
    const TAGS_MEMBERS_BATCHTAGGING     = '/tags/members/batchtagging?'; //批量为用户打标签
    const TAGS_MEMBERS_BATCHUNTAGGING   = '/tags/members/batchuntagging?'; //批量为用户取消标签
    const TAGS_GETIDLIST                = '/tags/getidlist?'; //获取用户身上的标签列表

    //一物一码相关
    const MARKETCODE_PREFIX         = 'https://api.weixin.qq.com/intp/marketcode';
    const MARKETCODE_APPLY          = '/applycode?';
    const MARKETCODE_APPLY_QUERY    = '/applycodequery?';
    const MARKETCODE_APPLY_DOWNLOAD = '/applycodedownload?';
    const MARKETCODE_ACTIVE         = '/codeactive?';
    const MARKETCODE_ACTIVE_QUERY   = '/codeactivequery?';
    const MARKETCODE_COED_TICKET    = '/tickettocode?';

    //
    const WXOPEN_WXAMPLINKGET    = '/wxopen/wxamplinkget?';
    const GET_API_DOMAIN_IP    = '/get_api_domain_ip?';
	...
	public function valid($return=false)
    {
        ...
        if ($_SERVER['REQUEST_METHOD'] == "POST") {
            ...
            if ($this->encrypt_type == 'aes') { //aes加密
            	...
            	$pc = new \Nanxihang\Wechat\Internal\Prpcrypt($this->encodingAesKey);
    ...
    /**
     * 用于第三方代码，收到数据解码后，来对$this->_receive赋值，保证以后流程运行
     * @param $arrData
     */
    public function setRev($arrData){
        $this->_receive = $arrData;
    }
...
<Nonce><![CDATA[%s]]></Nonce>
     ...
     * @return array|bool 返回签名字串
     */
    public function getCardSign($card_type='',$card_id='',$code='',$location_id='',$timestamp=0, $noncestr='', $appid=null){
        ...
        $arrdata = array("api_ticket" => $this->api_ticket,"app_id" => $this->appid,"card_id" => $card_id,"code" => $code,"card_type" => $card_type,"location_id" => $location_id,"timestamp" => $timestamp, "nonce_str" => $noncestr );
	...
	public function checkMenu(){
		if (!$this->access_token && !$this->checkAuth()) return false;
		$result = $this->http_get(self::API_URL_PREFIX.self::MENU_CHECK_URL.'access_token='.$this->access_token);
		if ($result)
		{
			$json = json_decode($result,true);
			if (!$json || isset($json['errcode'])) {
     ...
     * @author: SongYang
     */
    public function getDraftList($offset,$count){
        if (!$this->access_token && !$this->checkAuth()) return false;
        $data = array(
            'offset' => $offset,
            'count' => $count,
        ...
        $result = $this->http_post(self::API_URL_PREFIX.self::DRAFT_BATCHGET_URL.'access_token='.$this->access_token,self::json_encode($data));
        if ($result)
        {
            $json = json_decode($result,true);
            if (isset($json['errcode'])) {
     ...
     * @author: SongYang
     */
    public function getFreepublishList($offset,$count){
        if (!$this->access_token && !$this->checkAuth()) return false;
        $data = array(
            'offset' => $offset,
            'count' => $count,
        ...
        $result = $this->http_post(self::API_URL_PREFIX.self::FREEPUBLISH_BATCHGET.'access_token='.$this->access_token,self::json_encode($data));
        if ($result)
        {
            $json = json_decode($result,true);
            if (isset($json['errcode'])) {
     ...
     * @author: SongYang
     */
    public function addDraft($arrData){
        if (!$this->access_token && !$this->checkAuth()) return false;
        $arrDraft = [];
        array_push($arrDraft,$arrData);
        $data['articles'] = $arrDraft;
        $result = $this->http_post(self::API_URL_PREFIX.self::DRAFT_ADD_URL.'access_token='.$this->access_token,self::json_encode($data));
        if ($result)
        {
            $json = json_decode($result,true);
            if (isset($json['errcode'])) {
     ...
     * @author: SongYang
     */
    public function getDraftrCount(){
        if (!$this->access_token && !$this->checkAuth()) return false;
        $result = $this->http_get(self::API_URL_PREFIX.self::DRAFT_COUNT_URL.'access_token='.$this->access_token);
        if ($result)
        {
            $json = json_decode($result,true);
            if (isset($json['errcode'])) {
    ...
    public function getTags() {
        if (!$this->access_token && !$this->checkAuth()) return false;
        $result = $this->http_get(self::API_URL_PREFIX.self::TAGS_GET.'access_token='.$this->access_token);
        if ($result)
        {
            $json = json_decode($result,true);
            if (isset($json['errcode'])) {
    ...
    public function createTag($name){
        ...
        $result = $this->http_post(self::API_URL_PREFIX.self::TAGS_CREATE.'access_token='.$this->access_token,self::json_encode($data));
        if ($result)
        {
            $json = json_decode($result,true);
            if (!$json || !empty($json['errcode'])) {
     ...
     * @return bool|mixed
     */
    public function updateTag($nId,$sName){
        if (!$this->access_token && !$this->checkAuth()) return false;
        $data = array(
            'tag'=>array(
                'id'=>$nId,
        ...
        $result = $this->http_post(self::API_URL_PREFIX.self::TAGS_UPDATE.'access_token='.$this->access_token,self::json_encode($data));
        if ($result)
        {
            $json = json_decode($result,true);
            if (!$json || !empty($json['errcode'])) {
     ...
     * @return bool|mixed
     */
    public function deleteTag($nId){
        if (!$this->access_token && !$this->checkAuth()) return false;
        $data = array(
            'tag'=>array('id'=>$nId)
        );
        $result = $this->http_post(self::API_URL_PREFIX.self::TAGS_DELETE.'access_token='.$this->access_token,self::json_encode($data));
        if ($result)
        {
            $json = json_decode($result,true);
            if (!$json || !empty($json['errcode'])) {
     ...
     * @return bool|mixed
     */
    public function getTagsUser($nTagId,$sNextOpenid){
        if (!$this->access_token && !$this->checkAuth()) return false;
        $data = array(
            'tagid' => $nTagId,
            'next_openid' => $sNextOpenid
        );
    ...
    public function getWxampLink()
    {
        if (!$this->access_token && !$this->checkAuth()) return false;
        $data = array();
        $result = $this->http_post(self::API_URL_PREFIX . self::WXOPEN_WXAMPLINKGET . 'access_token=' . $this->access_token, self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
    ...
    public function getApiDomainIP()
    {
        if (!$this->access_token && !$this->checkAuth()) return false;
        $data = array();
        $result = $this->http_post(self::API_URL_PREFIX . self::GET_API_DOMAIN_IP . 'access_token=' . $this->access_token, self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
     ...
     * @return bool|mixed
     */
    public function getUserTags($sOpenid){
        if (!$this->access_token && !$this->checkAuth()) return false;
        $data = array(
            'openid' => $sOpenid
        );
     ...
     * @return bool|mixed
     */
    public function setMemberBatchtagging($arrOpenid,$nTagId){
        if (!$this->access_token && !$this->checkAuth()) return false;
        $data = array(
            'openid_list' => $arrOpenid,
            'tagid' => $nTagId,
        );
     ...
     * @return bool|mixed
     */
    public function setMemberUnBatchtagging($arrOpenid,$nTagId){
        if (!$this->access_token && !$this->checkAuth()) return false;
        $data = array(
            'openid_list' => $arrOpenid,
            'tagid' => $nTagId,
	 ...
	 */
	public function addTemplateMessage($tpl_id, $keyword_name_list){
	    $data = array ('template_id_short' =>$tpl_id, 'keyword_name_list' => $keyword_name_list);
	    if (!$this->access_token && !$this->checkAuth()) return false;
    ...
    {
        if (!$this->access_token && !$this->checkAuth()) return false;
        $result = $this->http_get(self::API_URL_PREFIX . self::TEMPLATE_GET_ALL_TPL_URL . 'access_token=' . $this->access_token);
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    public function deleteTemplateMessage($data){
        if (!$this->access_token && !$this->checkAuth()) return false;
     ...
     * @return false|mixed
     */
    public function getCategorySubscribe(){
        if (!$this->access_token && !$this->checkAuth()) return false;
        $result = $this->http_get(self::API_BASE_URL_PREFIX . self::SUBSCRIBE_GET_TPL_CATEGORY_URL . 'access_token=' . $this->access_token);
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    public function getPubTemplateKeyWordsById($tid){
        $data = ['tid' => $tid];
        if (!$this->access_token && !$this->checkAuth()) return false;
        $result = $this->http_post(self::API_BASE_URL_PREFIX.self::SUBSCRIBE_GET_TPL_KEYWORDS_URL.'access_token='.$this->access_token,self::json_encode($data));
        if($result){
            $json = json_decode($result,true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    public function getPubTemplateTitleList($ids, $start, $limit){
        $data = array ('ids' => $ids, 'start' => $start, 'limit' => $limit);
        if (!$this->access_token && !$this->checkAuth()) return false;
        $result = $this->http_post(self::API_BASE_URL_PREFIX.self::SUBSCRIBE_GET_TPL_TITLE_URL.'access_token='.$this->access_token,self::json_encode($data));
        if($result){
            $json = json_decode($result,true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
     ...
     * @return array
     */
	public function addTemplateSubscribe($tid, $kidList, $sceneDesc){
        $data = array ('tid' =>$tid, 'kidList' => $kidList, 'sceneDesc' => $sceneDesc);
        if (!$this->access_token && !$this->checkAuth()) return false;
        $result = $this->http_post(self::API_BASE_URL_PREFIX.self::SUBSCRIBE_ADD_TPL_URL.'access_token='.$this->access_token,self::json_encode($data));
        if($result){
            $json = json_decode($result,true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
     ...
     * @return array
     */
    public function getTemplateSubscribe()
    {
        if (!$this->access_token && !$this->checkAuth()) return false;
        $result = $this->http_get(self::API_BASE_URL_PREFIX . self::SUBSCRIBE_GET_ALL_TPL_URL . 'access_token=' . $this->access_token);
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    /**
     * 删除订阅通知模板
     * @param $data     ['priTmplId' => 'wDYzYZVxobJivW9oMpSCpuvACOfJXQIoKUm0PY397Tc']
     * @return array
     */
    public function deleteTemplateSubscribe($data){
        if (!$this->access_token && !$this->checkAuth()) return false;
        $result = $this->http_post(self::API_BASE_URL_PREFIX.self::SUBSCRIBE_DEL_TPL_URL.'access_token='.$this->access_token,self::json_encode($data));
        if($result){
            $json = json_decode($result,true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
     ...
     * @return boolean|array
     */
	public function sendTemplateSubscribe($data){
        if (!$this->access_token && !$this->checkAuth()) return false;
        $result = $this->http_post(self::API_URL_PREFIX.self::SUBSCRIBE_SEND_URL.'access_token='.$this->access_token,self::json_encode($data));
        if($result){
            $json = json_decode($result,true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    public function unavailableCardCode($code,$card_id='') {
     ...
     public function getCardHtml($card_id='') {

        if ($card_id)
            $data['card_id'] = $card_id;
        if (!$this->access_token && !$this->checkAuth()) return false;
        $result = $this->http_post(self::API_BASE_URL_PREFIX . self::CARD_MPNEWS_GETHTML . 'access_token=' . $this->access_token, self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
     ...
     * 申请二维码接口
     * @param array $data 消息结构
     *              code_count  申请码的数量，10000的整数倍
     *              isv_application_id  外部单号，本地申请单号，是申请二维码关联标记
     * @return boolean|array
     */
    public function applyCode($data){
        if (!$this->access_token && !$this->checkAuth()) return false;
        $result = $this->http_post(self::MARKETCODE_PREFIX.self::MARKETCODE_APPLY.'access_token='.$this->access_token,self::json_encode($data));
        if ($result)
        {
            $json = json_decode($result,true);
            if (!$json || !empty($json['errcode'])) {
     ...
     * @return boolean|array
     */
    public function applyCodeQuery($data){
        if (!$this->access_token && !$this->checkAuth()) return false;
        $result = $this->http_post(self::MARKETCODE_PREFIX.self::MARKETCODE_APPLY_QUERY.'access_token='.$this->access_token,self::json_encode($data));
        if ($result)
        {
            $json = json_decode($result,true);
            if (!$json || !empty($json['errcode'])) {
     ...
     * @param array $data 消息结构
     *              application_id  申请单号
     *              code_start      开始位置 来自查询二维码申请接口
     *              code_end        结束位置 来自查询二维码申请接口
     * @return boolean|array
     */
    public function applyCodeDownload($data){
        if (!$this->access_token && !$this->checkAuth()) return false;
        $result = $this->http_post(self::MARKETCODE_PREFIX.self::MARKETCODE_APPLY_DOWNLOAD.'access_token='.$this->access_token,self::json_encode($data));
        if ($result)
        {
            $json = json_decode($result,true);
            if (!$json || !empty($json['errcode'])) {
     ...
     * @return boolean|array
     */
    public function codeActive($data){
        if (!$this->access_token && !$this->checkAuth()) return false;
        $result = $this->http_post(self::MARKETCODE_PREFIX.self::MARKETCODE_ACTIVE.'access_token='.$this->access_token,self::json_encode($data));
        if ($result)
        {
            $json = json_decode($result,true);
            if (!$json || !empty($json['errcode'])) {
     ...
     * @return boolean|array
     */
    public function codeActiveQuery($data){
        if (!$this->access_token && !$this->checkAuth()) return false;
        $result = $this->http_post(self::MARKETCODE_PREFIX.self::MARKETCODE_ACTIVE_QUERY.'access_token='.$this->access_token,self::json_encode($data));
        if ($result)
        {
            $json = json_decode($result,true);
            if (!$json || !empty($json['errcode'])) {
     ...
     * @return boolean|array
     */
    public function codeTicket($data){
        if (!$this->access_token && !$this->checkAuth()) return false;
        $result = $this->http_post(self::MARKETCODE_PREFIX.self::MARKETCODE_COED_TICKET.'access_token='.$this->access_token,self::json_encode($data));
        if ($result)
        {
            $json = json_decode($result,true);
            if (!$json || !empty($json['errcode'])) {
...
class WXBizMsgCrypt
{
    ...
    public function encryptMsg($replyMsg, $timeStamp, $nonce, &$encryptMsg)
    {
        ...
        return \Nanxihang\Wechat\Internal\ErrorCode::$OK;
    }
    ...
    public function decryptMsg($msgSignature, $timestamp, $nonce, $postData, &$msg)
    {
        if (strlen($this->encodingAesKey) != 43) {
            return \Nanxihang\Wechat\Internal\ErrorCode::$IllegalAesKey;
        }
        ...
        if ($signature != $msgSignature) {
            return \Nanxihang\Wechat\Internal\ErrorCode::$ValidateSignatureError;
        }
...
class SHA1
{
    ...
    public function getSHA1($token, $timestamp, $nonce, $encrypt_msg)
    {
        ...
        try {
            ...
            $str = implode($array);
            return array(\Nanxihang\Wechat\Internal\ErrorCode::$OK, sha1($str));
        } catch (\Exception $e) {
            //print $e . "\n";
            return array(\Nanxihang\Wechat\Internal\ErrorCode::$ComputeSignatureError, null);
        }
    }
...
class XMLParse
{
    ...
    public function extract($xmltext)
    {
        try {
            ...
            //print $e . "\n";
            return array(\Nanxihang\Wechat\Internal\ErrorCode::$ParseXmlError, null, null);
        }
    }
